...
\begin{itemize}
  ...
  \item When the number of samples is small, we should consider leave-one-out.
  \item When the number of samples is very large, we must consider the cost of training, 
  and how well the model scale with larger data. 
\end{itemize}
...
\begin{itemize}
  ...
  \item Final ROC plot:
    ...
    \end{figure}
  \item AUC is the area under the ROC curve. We can use {\em Riemann sum} to compute it.
\end{itemize}
...
\begin{itemize}
  ...
  \item Since we have already selected the model, we don't need to do a train-test split. We want to use all the data to train our model before shipping it.
  ...
  \item \textbf{Note: } Decision Tree does not scale well with very large data set. 
  We will learn neural network later in this course, which scales better with larger data set.
\end{itemize}
